---
title: |
  Title: Tumour size and overall survival in a cohort of patients with unifocal glioblastoma: a univariable prognostic modelling and resampling study
author:
  - Kavi Fatania:
...
Various glioblastoma (GBM) prognostic models incorporating tumour volume or diameter have been proposed - often with inconsistent performance. The aim was to investigate the relationship between overall survival (OS) and tumour size using a well curated dataset of patients with GBM.

[Methods]{.underline}

279 patients with IDH-wildtype unifocal WHO grade 4 GBM and pre-operative MRI between 2014-2020 were retrospectively included. Tumours were segmented using deep-learning with manual correction and manual diameter measurements. Association between core volume, whole volume (CV/WV) and diameter with OS was evaluated using univariable Cox proportional hazard models. Log transformation and penalised splines were used to assess non-linearity. Resampling with 1,000,000 repetitions and varying sample size was used to create Cox models and percentage of significant models was calculated.

[Results]{.underline}

Median (interquartile range) CV 28.1cm^3^ (12.6 – 50.3), WV 103.3cm^3^ (45.6 – 160.1) and diameter 4.4cm (3.3-5.4). No association with OS was observed with univariable models of size ± log transformation or with non-linear modelling. Resampling led to higher percentages of significant models (p<0.05) when CV or WV was log transformed, and for volume and diameter using larger samples. For n=50, 5.1% of WV vs 5.2% log(WV), 5.0% CV vs 7.0% log(CV) were significant. At n=279, 5.6% WV and 9.7% log(WV), 8.6% CV vs 8.9% log(CV) had p<0.05.

[Conclusion]{.underline}

Although an association between tumour size and OS was not demonstrated initially in this well-curated institutional dataset, resampling suggests a univariable prognostic effect may become apparent with very large datasets and specific data (log) transformations.
...
* Prognostic modelling in GBM should include data transformation and appropriate sample sizes 


[Summary statement]{.underline}

**Glioblastoma size is often considered for inclusion into multivariable models based upon univariable association to overall survival, but in our cohort of GBM patients and use of resampling we find that this approach can be affected by the sample size and data transformation.**
...
**[Introduction]{.underline}**(400-600 max)
...
In **Supplementary Figure 2a-c**, each tumour size parameter (with and without log-transformation) is plotted against the log-HR. The fit of a linear function to the data is compared with the use of a penalised splines, and these suggest that there is limited evidence that the tumour size parameters have a univariable prognostic relationship - the model closely following the reference line for linear and non-linear functions. These results show that within our single institutional cohort, there is no evidence to support a univariable linear or non-linear prognostic relationship between OS and size.


In multivariable analyses, however, there is evidence of a prognostic association between size and OS when adjusting for clinical variables. A summary of the association of size variables in multivariable models with OS is shown in **Table \@ref(tab:bivar)**. CV, WV, log(WV) and diameter adjusted for type of surgery show a statistically significant association with OS, and although not significant at the 0.05 level, the HRs for log(CV) and log(diameter) are relatively wide, especially the latter indicating uncertainty in the HR estimate. Similarly, for the model adjusted for all clinical variables, only diameter remains statistically significant at the 0.05 threshold, however the HR for log(CV), log(WV) and log(diameter) do suggest a potentially prognostic effect with relatively wider confidence intervals (and less certainty) for HR estimate of the latter two variables. The univariable and multivariable prognostic associations of each clinical variable to OS is provided in **Supplementary Tables 2a-b**. Our cohort of GBM does therefore suggest that size is associated with OS in multivariable models and whilst several parameters do not achieve statistical significance there is supportive evidence of a potential prognostic relationship.  


```{r, Bivariable, include=FALSE, echo=FALSE}
```

```{r bivar, tab.cap='Table of the prognostic effect of each tumour size parameter (Whole volume, core volume and diameter) within multivariable models predicting overall survival that have been adjusted for selected clinical variables (n=279)', echo=FALSE}
ft_univ
```
...
The results of the univariable resampling study are shown in **Table \@ref(tab:simresults)**. For all size parameters, higher percentages of models with p\<0.05 were seen as the sample size increased, and for tumour volume (CV or WV), also after log transformation. 
