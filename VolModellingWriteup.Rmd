...
**Figure \@ref(fig:boxplot_diam) - \@ref(fig:boxplot_core)** show the distributions of the p-values extracted from models during the resampling study. For CTV and WTV, but not diameter, there is downwards trend as sample size increases, suggesting that the probability of seeing a prognostic effect is increased as sample size increases. Results from the resampling study indicate that the effect of decreasing the p-value distribution, and thereby increasing the probability of demonstrating a prognostic effect of size, by log-transformation is greater for WTV than CTV across the range of sample sizes.


```{r, Boxplotfunction, include=FALSE, echo=FALSE}
```

```{r boxplot_diam,fig.cap='Boxplots of p-values from resampling study for tumour diameter models',echo=FALSE, dpi=300,fig.height=7.5,fig.width=10,out.height=450,out.width=600}
# GrpBxplot(diam,log.diam,Samp_sizes,"Tumour Diameter")
```

```{r boxplot_wtv,fig.cap='Boxplots of p-values from resampling study for whole tumour volume (WTV) models', echo=FALSE}
# GrpBxplot(wtv,log.wtv,Samp_sizes,"Whole tumour volume")
```

```{r boxplot_core,fig.cap='Boxplots of p-values from resampling study for core tumour volume (CTV) models', echo=FALSE}
# GrpBxplot(core,log.core,Samp_sizes,"Core tumour volume")
```

**Figure \@ref(fig:boxplot_diam) - \@ref(fig:boxplot_core)** -- Each plot shows the distribution of p-values extracted from each model created across the 1,000,000 repetitions for each sample size. For each tumour size parameter, the p-value distributions for models before and after log-transformation are shown side by side (see figure legends). Dotted lines indicate the 0.05 significance level. 
...
KF is a 4ward North Clinical PhD fellow funded by Wellcome award 203914/Z/16/Z. Salary for AFS & SC is supported by the Leeds Hospitals Charity and Leeds RadNET. Salary for JOC is supported by Cancer Research
UK Advanced Clinician Scientist Fellowship (C19221/A22746).
